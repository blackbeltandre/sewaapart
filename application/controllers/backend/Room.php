...
class Room extends CI_Controller { 
		function __construct()
	{
        ...
        $this->load->model(array('m_login','room_model'));   
        $this->load->database();
	}
function index()
	{
    ...
    {    $data = array();
        ...
        $data['list_room'] = $this->room_model->show_list_room()->result_array();
		$data['title'] = ' INSERT ROOM';   
        $data['main'] = 'backend/Room';
...
function proses()
   {
    ...
    {    $data = array();
        ...
        $this->form_validation->set_rules('id_category', 'Category Room', 'required|min_length[1]|max_length[4]');
        $this->form_validation->set_rules('bahasa', 'Bahasa', 'required|min_length[1]|max_length[4]');
        if ($this->form_validation->run() == FALSE)
    {
        ...
        $this->db->set("id","NEXTVAL('sequence_gallery_room')",FALSE);
		$create = $this->db->insert('room',$data);
    ...
    {    $data = array();
         ...
         $data['pengguna'] = $this->m_login->dataPengguna($user);
           ...
           $multiple = $this->room_model->multiple_delete();
  ...
  redirect('backend/room/cek');     
    ...
    {    $data = array();
        ...
        $data['category_room'] = $this->room_model->show_category_room()->result_array();
		$data['single_room'] = $this->room_model->show_single_room_id($id);
		$data['title'] = ' EDIT ROOM';   
        $data['main'] = 'backend/Edit_room';
        $this->load->view('backend/Main_v', $data);
...
function update_room($id) 
    ...
    {   
        ...
        $config['base_url'] = site_url('backend/room/cek');
       ...
       $config['first_link']       = 'First';
        ...
        $data['room'] = $this->room_model->get_room_list($config["per_page"], $data['page']);           
        $data['pagination'] = $this->pagination->create_links();    
        $data['title'] = ' LIST ROOM';   
        $data['main'] = 'backend/List_room';
        $this->load->view('backend/Main_v', $data);
      } 
      function delete($id){
        $this->db->where('id',$id);
        ...
        $this->db->delete('room', array('foto' => $foto));
        $this->db->where('id', $id);
