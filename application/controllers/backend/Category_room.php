<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
    class Category_room extends CI_Controller { 
		function __construct()
    	{
            ...
            $this->load->model(array('m_login','category_room_model'));   
	 ...
	 { if($this->session->userdata('isLogin') == FALSE)
        {
            ...
            {    
		        ...
		        $data['title'] = ' INSERT CATEGORY ROOM';   
            $data['main'] = 'backend/category_room';
    ...
    function proses()
   {
        ...
        $this->db->set("id_category","NEXTVAL('sequence_category_room')",FALSE);
        $create  = $this->db->insert('category_room',$data);
        if ($create) $this->session->set_flashdata('message', "<div class='alert alert-dismissable alert-info'><button type='button' class='close' data-dismiss='alert'>
             ...
             $this->session->set_flashdata('message', "<div class='alert alert-info alert-block'><button type='button' class='close' data-dismiss='alert'>
                ...
                redirect('backend/category_room/cek'); 	
            }
        }
       
            function edit_category_room()
	{
        if($this->session->userdata('isLogin') == FALSE)
        {
            redirect('backend/login/login_form');
            ...
            {    
                $data = array();
		            ...
		            $data['single_category_room'] = $this->category_room_model->show_cat_room_id($id_category);
		      $data['title'] = ' EDIT CATEGORY ROOM';   
            $data['main'] = 'backend/Edit_category_room';
        $this->load->view('backend/Main_v', $data);
    }
    function update_category_room() 
      {
        if($this->session->userdata('isLogin') == FALSE)
        {
            redirect('backend/login/login_form');
		...
		$update = $this->db->update('category_room',$data);
        ...
        $config['base_url'] = site_url('backend/category_room/cek');
        // $count = $this->db->query("select count(*) from asset_user")->result();
        // var_dump($count);
        $config['total_rows'] = $this->db->count_all("category_room");
        $config['per_page'] = "10";
       ...
       $config['first_link']       = 'First';
        ...
        $data['category_room'] = $this->category_room_model->get_category_room_list($config["per_page"], $data['page']); 
        $data['pagination'] = $this->pagination->create_links();
        $data['title'] = ' LIST CATEGORY ROOM';   
        $data['main'] = 'backend/List_category_room';
        $this->load->view('backend/Main_v', $data);
      } 
      function delete_multiple() {
        if($this->session->userdata('isLogin') == FALSE)
        {
           ...
           $multiple = $this->category_room_model->multiple_delete();
      ...
      function delete($id_category){
        ...
        $delete =  $this->db->delete('category_room');
