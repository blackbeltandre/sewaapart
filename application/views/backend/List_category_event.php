  ...
  <div class="col-lg-12">
                        <div class="card">
                            ...
                            <div class="card-body card-block">
                              <form action="<?php echo base_url(); ?>backend/category_event/cek" method="post">
             <?php echo $this->session->flashdata('message'); ?>
                    <div class="col-md-12">
                        <div class="panel panel-info" id="demo">
                            <div class="panel-heading">
                                <h3 class="panel-title"><span class="panel-icon mr5"><i class="ico-users"></i></span> CATEGORY EVENT</h3>
                                <div class="panel-toolbar text-right">
                                </div>
                            </div>
                                <div class="panel-toolbar text-right">
                                    <div class="btn-group">
                                   <a type="button" class="btn btn-sm btn-success" data-toggle="tooltip" data-placement="top" title="Add data?" href="<?php echo base_url('backend/category_event');?>"><i class="icon ico-pencil"></i> Add Data ?</a>
                                </div>
                            </div>
                            <div class="table-responsive panel-collapse pull out">
                               <div class="col-md-3">
                                  <select class="form-control mt4" name="action">
                                    ...
                                    <input type="submit" id="selected" name="selected" class="btn btn-s-md btn-info" formaction="<?php echo base_url(); ?>backend/category_event/delete_multiple" value="Proses ?" ><p align="right"></p>
                          <script>
                                  jQuery(function ($) {
               ...
               <input type="checkbox" name="selected[]" id="selected-<?php echo $selected; ?>" class="selected" value="<?php echo $category[$i]["id_category"]; ?>" data-toggle="checkall" data-target="#table1">  
                   <th  style="width:5%"><?php echo ($page+$i+1); ?></th>
                          ...
                          <th  style="width:30%"><?php echo $category[$i]["category"]; ?></th>
                           <th  style="width:5%"><?php if($category[$i]["bahasa"]=="ENG"){
                            echo "<p class='btn-success'>INGGRIS</p>";
                            }
                            else{
                              echo "<p class='btn-info'>INDONESIA</p>";
                            } ?></th>
                        <th  style="width:10%">   <center><a type="button" class="btn btn-sm btn-warning" data-toggle="tooltip" data-placement="left" title="Update data?" href="<?php echo base_url('backend/category_event/edit_category');?>/<?php echo $category[$i]["id_category"] ?>"><font color="white">UBAH</font></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                      <a type="button" class="btn btn-sm btn-danger" data-toggle="tooltip" data-placement="bottom" title="Delete Data?" href="<?php echo base_url('backend/category_event/delete');?>/<?php echo $category[$i]["id_category"] ?>"onClick="return confirm('Anda Yakin Akan Menghapus Data Ini?');">HAPUS</a></center></th>
                   </tr> 
                   <?php
                    $selected++;
                       $selected = $selected++;
